...
class Ads extends MY_Controller {
    ...
    function add($client_id=null) {
        ...
        $this->output->js('https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.15.1/moment.min.js');

        $this->output->js('https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/js/bootstrap.min.js');

        $this->output->js('https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datetimepicker/4.17.37/js/bootstrap-datetimepicker.min.js');

        $this->output->css('https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datetimepicker/4.17.37/css/bootstrap-datetimepicker.css');
            ...
            $image_upload=$this->ad->multiple_image_upload('userfile');
             ...
             //  $filename = $image_upload['file_name'];
               ...
               // $rotate = imagerotate($source, $degrees, 0);
                //imagejpeg('uploads/documents/'.$rotate);
               // exit();
                ...
                // $video_link_description=$this->input->post('video_link_description');

                // echo $video_link_description;

                // $video_link_image=$this->input->post('video_link_image');

                // echo $video_link_image;

                // $video_link_title=$this->input->post('video_link_title');

                // echo $video_link_title;
                ...
                $active_time = $this->input->post('active_date');
                
                $mdate = date("Y-m-d");

                if($active_time>=$mdate  ){
                	 $status = 0 ;
                

                }else if ($status==='0' && $active_time<=$mdate ){
                ...
                $av = '';
                if($avatar==null){
                    $av = 'https://easyhealthlink.livinglinks.city/uploads/icon/default.jpg';
                }else{
                    $av = $avatar ;
                }
                $params=array('choose'=> $this->input->post('choose'),
                ...
                'default_rd'=>$this->input->post('default_rd'),
                  
                 'active_on_time'=>$this->input->post('active_date'),  
                ...
                'web_link_1_show_hide'=>$this->input->post('Weblink_show_1'),

                'web_link_2_show_hide'=>$this->input->post('Weblink_show_2'),

                'web_link_3_show_hide'=>$this->input->post('Weblink_show_3'),

                'merchant_address'=>$this->input->post('merchant_address'),
               
                'category_id'=> $this->input->post('category_id'),
                'image'=> $image,
                'degree'=>json_encode( $this->input->post('rotate')),
                'avatar'=>$av,
                'status'=>$status,
                'tags'=> $tags,

                'video_link_link'=> $this->input->post('link'),
                ...
                'appointment_date'=> json_encode($this->input->post('appointment_date')),
            ...
            if($this->ad->verify_validation()) {
                ...
                if($this->session->userdata('logged_in')->user_role==1) {
                   ...
                   // redirect('users/adds_listing?client_id='.$client_id);
                    redirect('https://easyhealthlink.livinglinks.city/ads/index');
                }
